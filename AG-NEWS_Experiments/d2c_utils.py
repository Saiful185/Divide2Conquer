...
def weight_scalling_factor(subset_trn_data, subset_no):
    subset_nos = list(subset_trn_data.keys())
    #get the bs
    bs = list(subset_trn_data[subset_no])[0][0].shape[0]
    #first calculate the total training data points across subsets
    central_data_count = sum([tf.data.experimental.cardinality(subset_trn_data[subset_no]).numpy() for subset_no in subset_nos])*bs
    # get the total number of data points held by a subset
...
def test_train_model(X_test, Y_test,  model, comm_round):
    cce = tf.keras.losses.CategoricalCrossentropy(from_logits=True)
    #logits = model.predict(X_test, batch_size=100)
    logits = model.predict(X_test)
    loss = cce(Y_test, logits)
